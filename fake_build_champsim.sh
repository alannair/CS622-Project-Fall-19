...
if [ "$#" -ne 8 ]; then
    ...
    echo "Usage: ./fake_build_champsim.sh [branch_pred] [l1i_pref] [l1d_pref] [l2c_pref] [llc_pref] [llc_repl] [num_core] [debugname]"
    exit 1
fi
...
cp fakestuff/fake_cache.cc src/cache.cc
cp fakestuff/fake_cache.h inc/cache.h
cp fakestuff/fake_base_replacement.cc replacement/base_replacement.cc
...
cp realstuff/cache.cc src/cache.cc
cp realstuff/cache.h inc/cache.h
cp realstuff/base_replacement.cc replacement/base_replacement.cc
...
echo "Cores: ${NUM_CORE}"
BINARY_NAME="fake-${BRANCH}-${L1I_PREFETCHER}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${LLC_REPLACEMENT}-${NUM_CORE}core-${DEBUGNAME}"
