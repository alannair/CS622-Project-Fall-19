...
#define L2C_RES_WAYS 4
#define L2C_FELLOW_GRP_SIZE 4
...
#define LLC_RES_WAYS 8
#define LLC_FELLOW_GRP_SIZE 4

class CACHE : public MEMORY {
  public:
    uint32_t cpu;
    const string NAME;
    const uint32_t NUM_SET, NUM_WAY, NUM_RES, NUM_LINE, WQ_SIZE, RQ_SIZE, PQ_SIZE, MSHR_SIZE, FELLOW_GRP_SIZE;
    uint32_t LATENCY;
    BLOCK **block;
    int fill_level;
    ...
    CACHE(string v1, uint32_t v2, int v3, uint32_t v4, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8)
        : NAME(v1), NUM_SET(v2), NUM_WAY(v3), NUM_LINE(v4), WQ_SIZE(v5), RQ_SIZE(v6), PQ_SIZE(v7), MSHR_SIZE(v8), NUM_RES(v3/2), FELLOW_GRP_SIZE(4) {

        LATENCY = 0;

        // if( cache_type == IS_L2C)
        // {
        //     NUM_RES = L2C_RES_WAYS;
        //     FELLOW_GRP_SIZE = L2C_FELLOW_GRP_SIZE;
        // }
        // else if( cache_type == IS_LLC)
        // {
        //     NUM_RES = LLC_RES_WAYS;
        //     FELLOW_GRP_SIZE = LLC_FELLOW_GRP_SIZE;
        // }
    ...
    uint32_t get_set(uint64_t address),
             get_fellow_set_id(uint64_t address),
