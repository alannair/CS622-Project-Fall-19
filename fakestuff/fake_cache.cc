...
uint32_t CACHE::get_fellow_set_id(uint64_t address)
{
    return (uint32_t) (get_set( address) / FELLOW_GRP_SIZE);
}
...
int CACHE::check_hit(PACKET *packet)
{
    ...
    if( match_way == -1 && cache_type >= 6)
    {
        uint32_t fellow_set = get_fellow_set_id( packet->address);

        for( uint32_t fset = fellow_set * FELLOW_GRP_SIZE; fset < (fellow_set + 1)*FELLOW_GRP_SIZE; ++fset )
        {
            if( set == fset) continue;

            for( uint32_t way = NUM_WAY - NUM_RES; way < NUM_WAY; ++way)
            {
                if (block[fset][way].valid && (block[fset][way].full_addr == packet->full_addr))
                {
                    DP ( if (warmup_complete[packet->cpu]) {
                    cout << "[" << NAME << "] " << __func__ << " instr_id: " << packet->instr_id << " type: " << +packet->type << hex << " addr: " << packet->address;
                    cout << " full_addr: " << packet->full_addr << " tag: " << block[fset][way].tag << " data: " << block[fset][way].data << dec;
                    cout << " set " << set << " fset: " << fset << " way: " << way << " lru: " << block[fset][way].lru;
                    cout << " event: " << packet->event_cycle << " cycle: " << current_core_cycle[cpu] << endl; });


                // swap this block with MRU block of set and leave LRU order within set unchanged
                BLOCK temp = BLOCK();
                ...
                if( i == NUM_WAY) assert(0);

                temp = block[set][i];
                block[set][i] = block[fset][way];
                block[fset][way] = temp;

                uint32_t templru;
                templru = block[set][i].lru;
                block[set][i].lru = block[fset][way].lru;
                block[fset][way].lru = templru;
...
int CACHE::invalidate_entry(uint64_t inval_addr)
{
    ...
    if( match_way == -1 && cache_type >= 6)
    {
        uint32_t fellow_set = get_fellow_set_id( inval_addr);
        ...
        for( uint32_t fset = fellow_set * FELLOW_GRP_SIZE; fset < (fellow_set + 1)*FELLOW_GRP_SIZE; ++fset )
        {
            ...
            for( uint32_t way = NUM_WAY - NUM_RES; way < NUM_WAY; ++way)
            {
                if (block[fset][way].valid && (block[fset][way].full_addr == inval_addr))
                {
                    DP ( if (warmup_complete[packet->cpu]) {
                    cout << "[" << NAME << "] " << __func__ << " instr_id: " << packet->instr_id << " type: " << +packet->type << hex << " addr: " << packet->address;
                    cout << " full_addr: " << packet->full_addr << " tag: " << block[fset][way].tag << " data: " << block[fset][way].data << dec;
